...
SOURCE_ALERT_DIR = r"D:\0-rtspspjk\backend\alert"
EXPORT_DIR = r"D:\0-rtspspjk\export_data_oracle"
...
def main():
    ...
    for i, row in enumerate(rows):
        ...
        pattern_alert = f"{cam_code}_{raw_label}_{time_str}"

        path_orig, name_orig = find_file_match(SOURCE_ORIG_DIR, pattern_orig)
        path_alert, name_alert = find_file_match(SOURCE_ALERT_DIR, pattern_alert)
        
        if path_orig or path_alert:
            csv_path_orig = ""
            csv_path_alert = ""
            ...
            if path_alert:
                shutil.copy2(path_alert, os.path.join(save_img_dir, name_alert))
                csv_path_alert = os.path.join("images", name_alert)
            ...
            csv_data.append({
                ...
                "置信度": confidence,
                "原图路径": csv_path_orig,
                "告警图路径": csv_path_alert,
                "原图文件名(新)": name_orig if name_orig else "",
                "告警图文件名(新)": name_alert if name_alert else ""
            })
